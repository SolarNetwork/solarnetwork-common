...
public class AuthorizationUtils {
	...
	public static byte[] computeHmacSha256(final byte[] key, final String msg) {
		return computeMacDigest(key, msg.getBytes(UTF_8), "HmacSHA256");
	}
	...
	public static byte[] computeMacDigest(final String secret, final String msg, String alg) {
		return computeMacDigest(secret.getBytes(UTF_8), msg.getBytes(UTF_8), alg);
	}
	...
	public static String uriEncode(CharSequence input) {
		...
		for ( int i = 0; i < input.length(); i++ ) {
			...
			if ( (ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z') || (ch >= '0' && ch <= '9')
				...
				byte[] bytes = String.valueOf(ch).getBytes(UTF_8);
				for ( byte b : bytes ) {
					tmpByteArray[0] = b;
	 ...
	 */
	public static String semiColonDelimitedList(String @Nullable [] list) {
