...
public final class Snws2AuthorizationBuilder
	...
	private @Nullable MultiValueMap<String, String> parameters;
	private @Nullable Set<String> signedHeaderNames;
	...
	public Snws2AuthorizationBuilder useSnDate(boolean useSnDate) {
		...
		if ( headers != null ) {
			...
			boolean hasDiscard = headers != null && headers.containsKey(discard);
	 ...
	 */
	@Override
	public Snws2AuthorizationBuilder date(@Nullable Instant date) {
		super.date(date);
	 ...
	 * @return this builder
	 */
	public Snws2AuthorizationBuilder parameterMap(@Nullable Map<String, String[]> params) {
	 ...
	 * @return this builder
	 */
	public Snws2AuthorizationBuilder queryParams(@Nullable Map<String, String> params) {
		MultiValueMap<String, String> map = null;
		if ( params != null ) {
			map = new LinkedMultiValueMap<String, String>(params.size());
			for ( Map.Entry<String, String> me : params.entrySet() ) {
	 ...
	 * @return this builder
	 */
	public Snws2AuthorizationBuilder signedHttpHeaders(@Nullable Set<String> signedHeaderNames) {
	...
	private void appendQueryParameters(StringBuilder buf) {
		Set<String> paramKeys = (parameters != null ? parameters.keySet() : Collections.emptySet());
		// parameters can not be null if paramKeys.size() >= 1, but re-check to avoid NullAway warning
		if ( parameters == null || paramKeys.size() < 1 ) {
			buf.append('\n');
			return;
		}
		...
		boolean first = true;
		for ( String key : keys ) {
			final List<String> vals = parameters.get(key);
			if ( vals == null ) {
				// can not really be null here, but re-check to avoid NullAway warning
				continue;
			}
			for ( String val : vals ) {
				if ( first ) {
