...
import static net.solarnetwork.codec.jackson.JsonUtils.readObject;
 ...
 * @since 4.44
 */
public class BasicDatumAuxiliaryRecordDeserializer extends StdScalarDeserializer<DatumAuxiliaryRecord> {

	/** A default instance. */
	public static final ValueDeserializer<DatumAuxiliaryRecord> INSTANCE = new BasicDatumAuxiliaryRecordDeserializer();
	...
	public @Nullable DatumAuxiliaryRecord deserialize(JsonParser p, DeserializationContext ctxt)
		...
		if ( t == JsonToken.VALUE_NULL ) {
			...
			throw MismatchedInputException.from(p,
					"Unable to parse DatumAuxiliaryRecord (not an object)");
		}
		...
		try {
			type = DatumAuxiliaryType.valueOf(json.path("type").stringValue());
		...
		if ( json.has("objectId") && json.has("kind") && json.has("sourceId") ) {
			...
			try {
				kind = ObjectDatumKind.fromValue(json.path("kind").stringValue());
			...
			datumIdentity = new DatumIdent(kind, json.path("objectId").asLong(),
					json.path("sourceId").stringValue(), ts);
		...
		final String notes = json.path("notes").stringValue();
