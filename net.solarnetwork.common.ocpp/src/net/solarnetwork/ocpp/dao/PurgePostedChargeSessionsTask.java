...
public class PurgePostedChargeSessionsTask implements Callable<Integer>, Runnable {
	...
	private @Nullable OptionalService<ChargeSessionDao> chargeSessionDao;
	...
	public Integer call() throws Exception {
		final ChargeSessionDao dao = OptionalService.service(chargeSessionDao);
		if ( dao == null ) {
			log.debug("No ChargeSessionDao available, cannot purge posted charge sessions.");
			return 0;
		}
		final Instant ts = Instant.now().minus(expirationHours, ChronoUnit.HOURS);
		log.debug(
				"Looking for OCPP posted charge sessions older than {} hours to purge (older that {})...",
				expirationHours, ts);
		final int result = dao.deletePostedChargeSessions(ts);
	 ...
	 *        the DAO to use
	 */
	public final void setChargeSessionDao(OptionalService<ChargeSessionDao> chargeSessionDao) {
	 ...
	 */
	public final void setExpirationHours(int expirationHours) {
