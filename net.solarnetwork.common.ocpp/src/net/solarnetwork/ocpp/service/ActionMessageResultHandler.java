...
public interface ActionMessageResultHandler<T, R> {
	 ...
	 * @param result
	 *        the successful result, or {@code null} if no result is available
	 * @param error
	 *        the error result, or {@code null} if no error occurred; can
	 *        implement {@link net.solarnetwork.ocpp.domain.ErrorHolder} to pass
	 *        specific details
	 * @return {@literal true} if the result was handled, {@literal false}
	 *         otherwise
	 */
	boolean handleActionMessageResult(ActionMessage<T> message, @Nullable R result,
