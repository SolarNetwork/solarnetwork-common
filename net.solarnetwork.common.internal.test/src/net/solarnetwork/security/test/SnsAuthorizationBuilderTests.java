...
public class SnsAuthorizationBuilderTests {
	...
	@Test
	public void example_computeSigningKey() {
		// GIVEN
		LocalDateTime date = LocalDateTime.of(2017, 1, 1, 0, 0, 0);

		// WHEN
		byte[] result = new SnsAuthorizationBuilder("bob@example.com")
				.computeSigningKey(date.toInstant(ZoneOffset.UTC), "ABC123");
		...
		then(result)
			.as("Result available")
			.isNotNull()
			.as("Encoded as HMAC+SHA")
			.isEqualTo(computeHmacSha256(computeHmacSha256("SNSABC123", "20170101"), "sns_request"))
			;
		// @formatter:on

		String h = Hex.encodeHexString(result);
		then(h).isNotNull();
	}
