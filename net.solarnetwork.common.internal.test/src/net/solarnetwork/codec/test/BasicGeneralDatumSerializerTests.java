...
public class BasicGeneralDatumSerializerTests {
	...
	@Test
	public void serializer_node() throws IOException {
		// GIVEN
		LocalDateTime date = LocalDateTime.of(2021, 8, 17, 14, 28, 12,
				(int) TimeUnit.MILLISECONDS.toNanos(345));
		Instant ts = date.toInstant(ZoneOffset.UTC);
	...
	@Test
	public void serializer_location() throws IOException {
		// GIVEN
		LocalDateTime date = LocalDateTime.of(2021, 8, 17, 14, 28, 12,
				(int) TimeUnit.MILLISECONDS.toNanos(345));
		Instant ts = date.toInstant(ZoneOffset.UTC);
		...
		s.putStatusSampleValue("c", "three");
		s.addTag("d");
		GeneralDatum datum = new GeneralDatum(DatumId.locationId(123L, "test.source", ts), s);
		...
		// @formatter:off
		assertThat("JSON", json,
				is("{\"created\":\"2021-08-17 14:28:12.345Z\",\"locationId\":123"
						+ ",\"sourceId\":\"test.source\""
						+ ",\"i\":{\"a\":1}"
						+ ",\"a\":{\"b\":2}"
						+ ",\"s\":{\"c\":\"three\"}"
