...
import static net.solarnetwork.web.jakarta.security.test.SecurityWebTestUtils.computeHMACSHA1;
import static net.solarnetwork.web.jakarta.security.test.SecurityWebTestUtils.httpDate;
...
import net.solarnetwork.web.jakarta.security.AuthenticationDataV1;
...
public class AuthenticationDataV1Tests {
	...
	public void missingDate() throws ServletException, IOException {
		...
		final String authHeader = createAuthorizationHeaderV1Value(TEST_AUTH_TOKEN, TEST_PASSWORD,
				request, new Date());
		try {
	...
	public void tooMuchSkew() throws ServletException, IOException {
		...
		request.addHeader("Date", now);
		String authHeader = createAuthorizationHeaderV1Value(TEST_AUTH_TOKEN, TEST_PASSWORD, request,
				now);
		request.addHeader(HTTP_HEADER_AUTH, authHeader);
		AuthenticationDataV1 authData = new AuthenticationDataV1(
				new SecurityHttpServletRequestWrapper(request, 1024),
